...
# Correct path to cascade file outside of src
cascade_path = os.path.join(os.path.dirname(__file__), '..', PATHS['cascade_file'])
face_cascade = cv2.CascadeClassifier(cascade_path)

if face_cascade.empty():
    logger.error(f"Failed to load cascade file at {cascade_path}")
    raise ValueError("Error loading cascade classifier")

if __name__ == '__main__':
    try:
        # Initialize camera and directories
        create_directory(PATHS['image_dir'])
        cam = initialize_camera(CAMERA['index'])
        ...
        logger.info(f"Initializing face capture for {face_name}")
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                if count < TRAINING['samples_needed']:
                    cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
                    face_img = gray[y:y + h, x:x + w]
                    img_path = os.path.join(PATHS["image_dir"], f'Users-{count + 1}.jpg')
                    cv2.imwrite(img_path, face_img)
                    count += 1
